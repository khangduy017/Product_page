import style from './FrameTra.module.css'
import React, { useState } from 'react'


function FrameTra(props) {
  ...
  const handleAdd = () => {
    ...
    if (amount1 > 0 || amount2 > 0) {
      setFeeInt((amount + 1) * 7 + (amount + 1) * props.price.slice(0, -1))
    }
    else {
  ...
  const handleSub = () => {
    if (amount > 1) {
      setAmount(amount - 1)
      if (amount1 > 0 || amount2 > 0) {
        setFeeInt(feeInt - props.price.slice(0, -1) - 7)
      }
      else {
  ...
  const handleAdd1 = () => {
    if (amount1 === 0 && amount2 === 0) setFeeInt(feeInt + 7 * amount)
    setAmount1(1)
    setAmount2(0)
    setTopping('Trân Châu Trắng')
  }
  ...
  return <div className={style.FrameTra}>
    ...
    <div className={style.information}>
      ...
      <div className={style.section3}>
        <div className={style.choose}>
          ...
          <div onClick={handleAdd1} style={{ cursor: 'pointer' }} className={(amount1 === 0) ? style.topping : style.toppingAdd}>
            <div className={style.infor}>
              <p className={style.name}>Trân Châu Trắng</p>
              <p style={{ fontWeight: '700' }}>+ 7.000đ</p>
            </div>
          ...
          <div onClick={handleAdd2} style={{ cursor: 'pointer' }} className={(amount2 === 0) ? style.topping : style.toppingAdd}>
            <div className={style.infor}>
              <p className={style.name}>Thạch Cà Phê</p>
              <p style={{ fontWeight: '700' }}>+ 7.000đ</p>
            </div>
        ...
        <div className={style.note}>
          ...
          <textarea placeholder={"Ghi chú......."} className={style.mainNote}></textarea>
        </div>
      </div>
      <div className={style.section4}>
        <button className={style.pay} onClick={() => pay([feeInt, amount, topping])}>{feeInt + '.000'}đ - Thêm vào giỏ hàng</button>
      </div>
    </div>
  </div>
}

export default FrameTra